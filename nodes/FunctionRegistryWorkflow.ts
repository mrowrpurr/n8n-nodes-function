...
class FunctionRegistryWorkflow {
	...
	private static activeCallStack: string[] = []
	...
	async callFunction(
		...
		// Create call stack identifier for recursion detection
		const callStackId = `${functionName}@${workflowId}`
		console.log("🔧 FunctionRegistryWorkflow: Call stack ID:", callStackId)
		console.log("🔧 FunctionRegistryWorkflow: Current call stack:", FunctionRegistryWorkflow.activeCallStack)

		// Check for recursion - if this exact function+workflow is already in the call stack
		if (FunctionRegistryWorkflow.activeCallStack.includes(callStackId)) {
			console.warn("🔧 FunctionRegistryWorkflow: ⚠️ Recursion detected! Function", functionName, "in workflow", workflowId, "is already in call stack")
			console.warn("🔧 FunctionRegistryWorkflow: ⚠️ Call stack:", FunctionRegistryWorkflow.activeCallStack)
			console.warn("🔧 FunctionRegistryWorkflow: ⚠️ Preventing infinite recursion")
			return { result: null, callId }
		}

		// Add to call stack
		FunctionRegistryWorkflow.activeCallStack.push(callStackId)
		console.log("🔧 FunctionRegistryWorkflow: Added to call stack:", callStackId)
		console.log("🔧 FunctionRegistryWorkflow: Updated call stack:", FunctionRegistryWorkflow.activeCallStack)
		...
		try {
			...
			// HYBRID APPROACH: Use pub/sub like simplified registry to avoid workflow execution
			console.log("🔧 FunctionRegistryWorkflow: Using pub/sub approach to avoid infinite recursion")
			
			try {
				// Send function call request via pub/sub to the process that registered the function
				...
				return new Promise(async (resolve) => {
					const timeout = setTimeout(async () => {
						if (!resolved) {
							resolved = true
							console.error(`🔧 FunctionRegistryWorkflow: Pub/sub call timed out for ${functionName} (callId: ${callId})`)
					...
					await subClient.subscribe(responseChannel, async (message) => {
						...
						try {
							...
							console.log(`🔧 FunctionRegistryWorkflow: Received pub/sub function response for ${functionName} (callId: ${callId}):`, response)
							await pubClient.disconnect()
							await subClient.disconnect()
							resolve({ result: response.result, callId })
						} catch (err) {
							console.error(`🔧 FunctionRegistryWorkflow: Error parsing pub/sub response:`, err)
							await pubClient.disconnect()
							await subClient.disconnect()
							resolve({ result: null, callId })
						}
					...
					console.log(`🔧 FunctionRegistryWorkflow: Publishing function call request to ${callChannel}:`, request)
					await pubClient.publish(callChannel, JSON.stringify(request))
				})
			} catch (error) {
				console.error("🔧 FunctionRegistryWorkflow: Error with pub/sub approach:", error)
			...
			const index = FunctionRegistryWorkflow.activeCallStack.indexOf(callStackId)
			if (index > -1) {
				FunctionRegistryWorkflow.activeCallStack.splice(index, 1)
				console.log("🔧 FunctionRegistryWorkflow: Removed from call stack:", callStackId)
				console.log("🔧 FunctionRegistryWorkflow: Updated call stack:", FunctionRegistryWorkflow.activeCallStack)
			}
		}
	}
